...
*/
/*
This file is based on pywin32 main.cpp
*/
...
TCHAR g_szEventSourceName[MAX_PATH] = _T("EPControl");
...
void WINAPI service_main(DWORD dwArgc, LPTSTR *lpszArgv)
{
	...
	if (g_serviceProcessFlags == SERVICE_WIN32_OWN_PROCESS) {
		...
		if (!pe->klass) {
			TCHAR svcInitBuf[256] = _T("epc.win32.service.EPControlSvc");
...
int PythonService_main(int argc, TCHAR **argv)
{
	...
	if ((argc > 1) && ((*argv[1] == '-') || (*argv[1] == '/'))) {
		if (_tcsicmp(_T("debug"), argv[1] + 1) == 0) {
			int argOffset = 1;
			bServiceDebug = TRUE;
			_tprintf(_T("Debugging service - press Ctrl+C to stop.\n"));
...
*
*************************************************************************/
static void ReportAPIError(DWORD msgCode, DWORD errCode)
{
	if (errCode == 0)
...
static void ReportPythonError(DWORD code)
{
	if (PyErr_Occurred()) {
		...
		if (szmbTraceback) {
			...
			// Because GetPythonTraceback use the deprecated strdup, we cannot free the memory without throwing a Heap corruption
			// FIXME: Patch Pypiwin32 to use _strdup!
			// free(szmbTraceback);
		}
...
static BOOL ReportError(DWORD code, LPCTSTR *inserts, WORD errorType)
{
	WORD numInserts = 0;
...
{
	PyObject *module, *f;
	PyObject *sitemod, *sitedict, *addsite, *siteargs;
	PyThreadState *threadState;
	HMODULE hmod;
	FARPROC proc;

	// Change the current directory to the exe path
	...
	// Set pylib as python home
	wchar_t python_home[] = L"pylib";
	Py_SetPythonHome(python_home);
	// Add pylib and pytool as Path
	Py_SetPath(L"pylib;pylib\\lib-dynload;pytool");
	
	// Start Python engine
	Py_Initialize();

	// Add pytool to site directory
	sitemod = PyImport_ImportModule("site");
	if (sitemod)
	{
		sitedict = PyModule_GetDict(sitemod);
		if (sitedict)
		{
			addsite = PyDict_GetItemString(sitedict, "addsitedir");
			if (addsite && PyCallable_Check(addsite))
			{
				siteargs = PyTuple_New(1);
				PyTuple_SetItem(siteargs, 0, PyUnicode_FromString("pytool"));
				PyObject_CallObject(addsite, siteargs);
				Py_DECREF(siteargs);
				Py_DECREF(addsite);
			}
			Py_DECREF(sitedict);
	...
	// Handle -c argument (for multiprocessing)
	...
	// Handle other command lines switches (except -debug)
	else if (argc >= 2 && _tcscmp(argv[1], _T("-debug")) != 0)
	{
		long ret = -1;
		PySys_SetArgv(argc, argv);

		PyObject *cmd_mod, *cmd_dict, *run_meth, *result;
		cmd_mod = PyImport_ImportModule("epc.win32.command_services");
		if (cmd_mod)
		{
